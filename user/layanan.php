...
$layanan = query("SELECT * FROM layanan_barber");
$profil = query("SELECT * FROM user_barber WHERE barber_id_user = '$id'")[0];
  ...
  <section class="layanan" id="layanan">
    <div class="container">
      <main class="contain" data-aos="fade-right" data-aos-duration="1000">
        <h2 class="text-head">Layanan <span>BABE </span>Barbershop</h2>
        <div class="row row-cols-1 row-cols-md-4">
          <?php foreach ($layanan as $row) : ?>
            <div class="col">
              <div class="card">
                <img src="../img/<?= $row["barber_foto"]; ?>" alt="gambar layanan" class="card-img-top">
                <div class="card-body text-center">
                  <h5 class="card-title"><?= $row["barber_nama"]; ?></h5>
                  <p class="card-text"><?= $row["barber_keterangan"]; ?></p>
                  <p class="card-price"><?= $row["barber_harga"]; ?></p>
                  <a href="jadwal.php?id=<?= $row["barber_id_layanan"]; ?>" type="button" class="btn btn-secondary">Jadwal</a>
                  <button type="button" class="btn btn-inti" data-bs-toggle="modal" data-bs-target="#pesanModal<?= $row["barber_id_layanan"]; ?>">Pesan</button>
                </div>
              </div>
            </div>

            <!-- Modal Pesan -->
            <div class="modal fade" id="pesanModal<?= $row["barber_id_layanan"]; ?>" tabindex="-1" aria-labelledby="pesanModalLabel<?= $row["barber_id_layanan"]; ?>" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    <h5 class="modal-title" id="pesanModalLabel<?= $row["barber_id_layanan"]; ?>">Pesan layanan <?= $row["barber_nama"]; ?></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                  </div>
                  <form action="" method="post">
                    <div class="modal-body">
                      ...
                      <div class="row justify-content-center align-items-center">
                        ...
                        <div class="text-center">
                          <h6 name="harga">Harga : <?= $row["barber_harga"]; ?></h6>
                        </div>
                        <div class="col">
                          <input type="hidden" name="id_lpg" class="form-control" id="exampleInputPassword1" value="<?= $row["barber_id_layanan"]; ?>">
                          <div class="mb-3">
                        ...
                        <div class="col">
                          <input type="hidden" name="harga" class="form-control" id="exampleInputPassword1" value="<?= $row["barber_harga"]; ?>">
                          <div class="mb-3">
